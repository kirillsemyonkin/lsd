...
import kirillsemyonkin.lsdata.LSD.Level;
import kirillsemyonkin.lsdata.LSD.List;
import kirillsemyonkin.lsdata.LSD.Option;
import kirillsemyonkin.lsdata.LSD.Option.None;
import kirillsemyonkin.lsdata.LSD.Option.Some;
import kirillsemyonkin.lsdata.LSD.KeyPathPart.Key;
import kirillsemyonkin.lsdata.LSD.KeyPathPart.Index;
import kirillsemyonkin.lsdata.LSD.ParseException;
import kirillsemyonkin.lsdata.LSD.ParseException.ExpectedKeyOrEnd;
import kirillsemyonkin.lsdata.LSD.ParseException.ExpectedKeyPartAfterKeySeparator;
import kirillsemyonkin.lsdata.LSD.ParseException.ExpectedLSDAfterKey;
import kirillsemyonkin.lsdata.LSD.ParseException.ExpectedListLSDOrEnd;
import kirillsemyonkin.lsdata.LSD.ParseException.KeyCollisionKeyAlreadyExists;
import kirillsemyonkin.lsdata.LSD.ParseException.KeyCollisionShouldBeLevelButIsNot;
import kirillsemyonkin.lsdata.LSD.ParseException.ReadFailure;
import kirillsemyonkin.lsdata.LSD.ParseException.UnexpectedCharAtFileEnd;
import kirillsemyonkin.lsdata.LSD.ParseException.UnexpectedCharEscapeEnd;
import kirillsemyonkin.lsdata.LSD.ParseException.UnexpectedCharInByteEscape;
import kirillsemyonkin.lsdata.LSD.ParseException.UnexpectedCharInUnicodeEscape;
import kirillsemyonkin.lsdata.LSD.ParseException.UnexpectedStringEnd;
import kirillsemyonkin.lsdata.LSD.PeekableChars;
import kirillsemyonkin.lsdata.LSD.Value;
 ...
 * }</pre>
 *
 * Visit <a href="https://github.com/kirillsemyonkin/lsd">LSD repository</a> for
...
public sealed interface LSD permits Value, List, Level {
        ...
        public Option<LSD> inner(Object... parts) {
            return switch (parts.length) {
                ...
                default -> {
                    ...
                    yield switch (key) {
                        case Index(var i) ->
                            Option.of(i >= 0 && i < get.size(), () -> get.get(i))
                            .flatMap(lsd -> lsd.inner(rest));
                        case Key(var ignored) ->
                            new None<>();
                    };
                }
            };
     ...
     */
    public sealed abstract class ParseException extends Exception permits
            ReadFailure,
    ...
    public sealed interface Option<T> permits Some, None {
        ...
        record None<T>() implements Option<T> {
            ...
            @Override
            public <T2, E extends Throwable> None<T2> mapThrowing(MapThrowing<T, T2, E> map) throws E {
                return new None<>();
            }
     ...
     */
    public static class PeekableChars {
     ...
     * @return Parsed LSD.
     * @throws kirillsemyonkin.lsdata.LSD.ParseException If unable to parse.
     */
    public static LSD parse(InputStream inputStream) throws ParseException {
     ...
     * @return Peeked character.
     * @throws kirillsemyonkin.lsdata.LSD.ParseException If unable to read.
     */
                ...
                case '\\' -> {
                    switch (read(stream).orElseThrow(UnexpectedCharEscapeEnd::new)) {
                        ...
                        case 'u' | 'U' -> {
                            ...
                            try {
                                var buf = ByteBuffer.wrap(new byte[]{firstSurrogate1, firstSurrogate2});
                            ...
                            try {
                                var buf = ByteBuffer.wrap(new byte[]{firstSurrogate1, firstSurrogate2, secondSurrogate1, secondSurrogate2});
                                result.append(UTF_16BE.newDecoder().decode(buf));
                            } catch (CharacterCodingException e) {
                                throw new UnexpectedCharInUnicodeEscape();
                            }
                        ...
                        default ->
                            throw new UnexpectedCharEscapeEnd();
                    }
                }
    ...
    private static Level readLevelInner(
        ...
        while (true) {
            ...
            switch (readKeyPath(stream)) {
                case Some(var k) ->
                    key = k;
                case None() when levelEndsWithClose ->
                    throw new ExpectedKeyOrEnd();
                case None() -> {
                    return results;
                }
            }
                        ...
                        switch (entry.getValue()) {
                            ...
                            case Level lvl -> {
                                switch (insertInto.get().computeIfAbsent(key, k -> new Level())) {
                                    ...
                                    case List ignored ->
                                        throw new KeyCollisionShouldBeLevelButIsNot();
                                    case Level insertInto2 ->
                                        mergeLevel(insertInto2, lvl);
                                }
                            }
                        }
                    }
    ...
    private static Option<ArrayList<String>> readKeyPath(PeekableChars stream) throws ParseException {
        ...
        while (true) {
            if (!(peek(stream) instanceof Some(var peek) && peek.get() == '.')) {
                break;
            }
            peek.accept();
     ...
     */
    public sealed interface KeyPathPart permits Key, Index {
